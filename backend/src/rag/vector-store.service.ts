...
export class VectorStoreService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    const dbUrl = this.config.get<string>('DATABASE_URL', 'file:./data/chat.db');
    const file = dbUrl.replace(/^file:/, '');
    const abs = path.isAbsolute(file) ? file : path.resolve(process.cwd(), 'prisma', file);
    ...
    this.stmts = {
      ...
      insVec: this.db.prepare(`INSERT INTO ${TABLE_VEC}(chunk_id, category, embedding) VALUES (?, ?, ?)`),
      delFts: this.db.prepare(`DELETE FROM ${TABLE_FTS} WHERE chunk_id = ?`),
      insFts: this.db.prepare(`INSERT INTO ${TABLE_FTS}(chunk_id, content, category) VALUES (?, ?, ?)`),
    };
  ...
  knn(opts: { embedding: Float32Array; k: number; category?: string }): Hit[] {
    const buf = Buffer.from(opts.embedding.buffer, opts.embedding.byteOffset, opts.embedding.byteLength);
    try {
      const rows = opts.category
        ? (this.stmts.knnCat.all(buf, opts.category, opts.k) as Array<{ chunk_id: number; distance: number }>)
        : (this.stmts.knn.all(buf, opts.k) as Array<{ chunk_id: number; distance: number }>);
      // vec0 devuelve distance (coseno o L2). Lo normalizamos: score = 1/(1+d)
      return rows.map(r => ({ chunkId: r.chunk_id, score: 1 / (1 + r.distance) }));
    } catch (e) {
  ...
  fts(opts: { query: string; k: number; category?: string }): Hit[] {
    ...
    try {
      const rows = opts.category
        ? (this.stmts.ftsCat.all(ftsQuery, opts.category, opts.k) as Array<{ chunk_id: number; score: number }>)
        : (this.stmts.fts.all(ftsQuery, opts.k) as Array<{ chunk_id: number; score: number }>);
